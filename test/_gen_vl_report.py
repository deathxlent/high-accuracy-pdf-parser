...
with open(r'C:\ws\high accuracy pdf parser\test\output_ocr_vl_gguf\llama_mtmd_output.txt', encoding='utf-8', errors='replace') as f:
    raw = f.read()

ocr_match = re.search(r'>\s*OCR:\s*\n(.+?)(?:\s*\[ Prompt:|\s*Exiting)', raw, re.DOTALL)
ocr_text = ocr_match.group(1).strip() if ocr_match else raw
