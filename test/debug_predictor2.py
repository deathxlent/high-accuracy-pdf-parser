...
try:
    print("\nCreating DocVLMLocalPredictor...")
    pp_option = PaddlePredictorOption(device="cpu")
    predictor = DocVLMLocalPredictor(
        model_dir=model_dir,
    ...
    if hasattr(tokenizer, 'chat_template') and tokenizer.chat_template:
        ...
        print(f"chat_template is callable: {callable(tokenizer.chat_template)}")
    ...
    print(f"\nHas <|IMAGE_START|>: {'<|IMAGE_START|>' in prompt}")
